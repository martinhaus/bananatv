...
class Plugin extends Container implements PluginContract
{
  ...
  public function boot()
  {
     ...
     *
      ...
      Description = "WPX Cron Manager Light is a WP Bones boilperate plugin"
      DomainPath = "localization"
      Name = "WPX Cron Manager Light"
      Network = false
      PluginURI = "http://undolog.com"
      TextDomain = "wpx-cron-manager-light"
      Title = "WPX Cron Manager Light"
      Version = "1.0.0"
    ...
    // Load text domain
    load_plugin_textdomain( "wpx-cron-manager-light", false, trailingslashit( basename( $this->basePath ) ) . $this->DomainPath );
    ...
    if ( ! empty( $menus ) && is_array( $menus ) ) {
      ...
      foreach ( $menus as $topLevelSlug => $menu ) {
        ...
        foreach ( $menu[ 'items' ] as $subMenu ) {
          ...
          $submenuSlug = isset( $submenuSlug ) ? md5( $topLevelSlug ) . '-' . sanitize_key( $subMenu[ 'page_title' ] ) : $topLevelSlug;
          ...
          if ( isset( $subMenu[ 'route' ][ 'load' ] ) ) {
            ...
            $func = create_function( '', sprintf( '$instance = new %s; return $instance->%s();',
                                                  'WPXCronManagerLight\\Http\\Controllers\\' . $controller, $method ) );
